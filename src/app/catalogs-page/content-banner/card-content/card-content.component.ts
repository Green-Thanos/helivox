...
export class CardContentComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.subscription = this.route.params.subscribe((params: Params) => {
      this.unloaded = true;
      this.data.getData(this.catalogCategory.catalog).subscribe(
        (response: any[]) => {
          this.catalogData = response === null ? [] : response;
          this.data.getData('Inputs').subscribe(
            (inputs: any[]) => {
              this.unloaded = false;
              this.inputs = inputs;
            },
  ...
  passesTags(i: number) {
    ...
    if (
      this.catalogData[i].category.toLowerCase() !== this.selected.toLowerCase()
    ) {
      return false;
    }
    if (
      !this.catalogData[i].labels.includes(this.activeFilters[2]) &&
      this.activeFilters[2] != 'Tags'
    ) {
      return false;
    }
    if (
      !this.checkIfInRange(
        ...
        this.catalogData[i].tags[0],
      ) &&
      this.activeFilters[0] != 'Hours'
    ) {
      return false;
    }
    if (
      this.activeFilters[1] != 'Cost' &&
      !this.checkIfInRange(this.activeFilters[1], this.catalogData[i].tags[1])
    ) {
      return false;
    }
    ...
    if (this.activeFilters[3] != 'Rating' && this.inputs[i] === undefined) {
      ...
      if (
        this.activeFilters[3] === 'Outstanding' &&
        !(this.inputs[i].rating >= 4)
      ) {
        return false;
      }
      if (
        this.activeFilters[3] === 'Superior' &&
        !(this.inputs[i].rating >= 3 && this.inputs[i].rating < 4)
      ) {
        return false;
      }
    }
