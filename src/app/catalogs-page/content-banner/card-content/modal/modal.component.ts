...
export class ModalComponent implements OnInit, OnDestroy {
  ...
  submitElement() {
    ...
    if (this.catalogForm.value.tag2 !== null) {
      labelsVal.push(this.catalogForm.value.tag2);
    }

    if (this.catalogForm.value.tag3 !== null) {
      labelsVal.push(this.catalogForm.value.tag3);
    }
    ...
    if (this.dta.getUser().role === 1) {
      ...
      this.dta.patchData(
        newDta,
        'Volunteer_Submissions/' + uid + '/' + this.dta.getVolQuests().length,
      );
      let newhrCount =
        this.dta.getUser().hours === undefined ||
        this.dta.getUser().hours === null
          ? 1
          : ++this.dta.getUser().hours;
      ...
      this.dta.patchData(
        newDta,
        this.filters.catalog + '/' + (this.allCatalogData.length - 1),
      );
    }
