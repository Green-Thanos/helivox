...
export class LoginPageComponent implements OnInit, OnDestroy {
  ...
  onSubmitLogin() {
    ...
    this.authSubscription = this.auth
      ...
      .subscribe(
        (resData) => {
          this.verificationSubscription = this.auth
            .fetchUserData(resData.idToken)
            .subscribe((userData) => {
              if (userData.users[0].emailVerified != true) {
                this.dta.setAlertData(
                  'User not verified: Re-signup',
                  true,
                  '#e65045',
                );
  ...
  onSubmitSignup() {
    ...
    this.authSubscription = this.auth
      .signup(
        ...
        this.signupForm.value.passwordData.password1,
      )
      .subscribe(
        (resData) => {
          this.verificationSubscription = this.auth
            .verification(resData.idToken)
            .subscribe(() => {
              this.dta.patchData(
                {
                  role: '0',
  ...
  onSubmitEmailReset() {
    ...
    this.unloaded = true;
    this.sendResetCodeSubscription = this.auth
      .sendPasswordResetCode(this.emailResetForm.value.username)
      .subscribe(
        () => {
