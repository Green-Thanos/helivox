...
export class AdminFeaturesComponent {
  ...
  ngOnInit(): void {
    if (this.dta.getUser().role >= 3) {
      ...
      this.http
        .get(
          'https://helivox-2-default-rtdb.firebaseio.com/Users.json/?auth=' +
            this.user.token,
        )
        .subscribe(
          (resData) => {
  ...
  checkIfValid(uid: string) {
    if (
      this.searchText !== '' &&
      !this.userList[uid].email.includes(this.searchText)
    ) {
      return false;
    }
    if (
      this.currentRoleSort !== -1 &&
      +this.userList[uid].role !== this.currentRoleSort
    ) {
      return false;
    }
    return true;
