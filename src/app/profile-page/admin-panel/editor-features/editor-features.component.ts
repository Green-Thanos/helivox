...
export class EditorFeaturesComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.http
      .get(
        'https://helivox-2-default-rtdb.firebaseio.com/Volunteer_Submissions.json?auth=' +
          this.dta.getUser().token,
      )
      .subscribe((volData) => {
  ...
  editVolData(approved: boolean, i: number) {
    if (approved) {
      this.dta.getData(this.volData[i].catalog).subscribe((data: any[]) => {
        this.dta.patchData(
          this.volData[i],
          this.volData[i].catalog + '/' + data.length,
        );
      });
    } else {
      for (
        let j = 0;
        j < this.volDataOriginal[Object.keys(this.volDataOriginal)[i]].length;
        j++
      ) {
        if (
          this.volData[i] ===
          this.volDataOriginal[Object.keys(this.volDataOriginal)[i]][j]
        ) {
          this.dta.deleteData(
            Object.keys(this.volDataOriginal)[i] + '/' + j,
