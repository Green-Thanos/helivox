...
export class AuthService {
  ...
  verification(idToken: string) {
    return this.http.post(
      'https://identitytoolkit.googleapis.com/v1/accounts:sendOobCode?key=AIzaSyDXcpYQ9XHpJSRyhhqJgRx4Mc99eh_MeLE',
      {
  ...
  autoLogin() {
    ...
    const loadedUser = new User(
      ...
      userData._role,
      userData._profile_picture,
      userData._name,
      userData._about,
      userData._hours,
    );
    if (
      localStorage.getItem('volData') !== null ||
      localStorage.getItem('volData') !== undefined
    ) {
  ...
  login(email: string, password: string) {
    return this.http
      .post<AuthResponseData>(
        'https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyDXcpYQ9XHpJSRyhhqJgRx4Mc99eh_MeLE',
        {
          email: email,
          password: password,
      ...
      .pipe(
        tap((resData) => {
          this.http
            .get(
              'https://helivox-2-default-rtdb.firebaseio.com/Users/' +
                resData.localId +
                '.json?auth=' +
                resData.idToken,
            )
            .subscribe(
              (userD: {
                ...
                if (+userD.role === 1) {
                  this.http
                    .get(
                      'https://helivox-2-default-rtdb.firebaseio.com/Volunteer_Submissions/' +
                        resData.localId +
                        '.json?auth=' +
                        resData.idToken,
                    )
  ...
  signup(email: string, password: string) {
    return this.http
      .post<AuthResponseData>(
        'https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyDXcpYQ9XHpJSRyhhqJgRx4Mc99eh_MeLE',
        {
          email: email,
          password: password,
          returnSecureToken: true,
  ...
  fetchUserData(idToken: string) {
    return this.http.post<UserResponseData>(
      'https://identitytoolkit.googleapis.com/v1/accounts:lookup?key=AIzaSyDXcpYQ9XHpJSRyhhqJgRx4Mc99eh_MeLE',
      {
        idToken: idToken,
      },
    );
  ...
  deleteUser(idToken: string) {
    this.http
      .post(
        'https://identitytoolkit.googleapis.com/v1/accounts:delete?key=AIzaSyDXcpYQ9XHpJSRyhhqJgRx4Mc99eh_MeLE',
        {
          idToken: idToken,
        },
      )
